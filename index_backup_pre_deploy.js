...
const dbPersistence = require('./database-persistence');
...
// Global user storage for profile persistence
let currentUserData = {
  1: { 
    id: 1, 
    first_name: "Jenny", 
...
try {
  ...
  if (fs.existsSync(userFile)) {
    ...
    currentUserData = { ...currentUserData, ...savedData };
...
function saveUserData() {
  try {
    ...
    const path = require('path');
    const userFile = path.join(__dirname, 'user_data.json');
    fs.writeFileSync(userFile, JSON.stringify(currentUserData, null, 2));
    console.log('💾 User data saved to file');
  } catch (error) {
    console.log('⚠️ Failed to save user data to file');
  }
}

// ============================================================================
// IN-MEMORY DATA STORE - Fix for data consistency issue
// ============================================================================
const dataStore = {
  users: new Map(),
  timeEntries: new Map(),
  ...
  init() {
    ...
    this.projects.set(1, { id: 1, name: 'API Development', description: 'Time tracking API development', client: 'Internal', status: 'active', color: '#4CAF50' });
    this.projects.set(2, { id: 2, name: 'Testing', description: 'API testing and QA', client: 'Internal', status: 'active', color: '#2196F3' });
    this.projects.set(3, { id: 3, name: 'Documentation', description: 'API documentation and guides', client: 'Internal', status: 'active', color: '#FF9800' });
...
app.get('/api/me', (req, res) => {
  ...
  const user = dataStore.users.get(userId) || dataStore.users.get(1);
...
app.post('/api/auth/register', (req, res) => {
  ...
  for (const [id, user] of dataStore.users) {
    if (user.email === email) {
      existingUser = user;
      break;
    }
  ...
  // Create new user with sequential ID (not random!)
  const userId = dataStore.nextUserId++;
  const newUser = {
    id: userId,
    userId: userId,
    tenantId: 1,
    employeeNumber: employeeNumber || `EMP${String(userId).padStart(3, '0')}`,
    firstName,
    lastName,
    email,
...
app.post('/api/auth/login', (req, res) => {
  ...
  // Find user by email in dataStore
  let user = null;
  for (const [id, u] of dataStore.users) {
    if (u.email === email) {
      user = u;
      break;
    }
  ...
  // If user not found, use default user
  if (!user) {
    user = dataStore.users.get(1);
  }
...
// Enhanced Timer Pause/Resume API
app.post('/api/me/timer/pause', async (req, res) => {
  const userId = 1; // Simplified for now
  const { action } = req.body; // 'pause' or 'resume'
  
  try {
    console.log(`⏸️ Timer ${action || 'toggle'} request for user ${userId}`);
    
    // Load current timer data from persistence
    const timerData = await dbPersistence.loadTimerData(userId);
    ...
    // Toggle pause state or follow explicit action
    ...
    await dbPersistence.saveTimerData(userId, timerData);
...
// Global storage for timers (enhanced with database persistence)
global.activeTimers = {};
global.stoppedTimersToday = {};
global.dailyWorkTime = {};

// Timer APIs
app.post('/api/me/timer/start', async (req, res) => {
  const { projectId, locationId, taskId, notes, force = false } = req.body || {};
  const userId = 1; // Simplified for this version
  ...
  try {
    // Check for existing active timer using database persistence
    const hasExistingTimer = await dbPersistence.hasActiveTimer(userId);
    console.log(`🔍 Existing timer check: ${hasExistingTimer}`);

    if (hasExistingTimer) {
      const existingTimerData = await dbPersistence.loadTimerData(userId);
      return res.status(400).json({
        success: false,
        message: 'Timer already running. Stop current timer first.',
        data: {
          currentTimer: existingTimerData,
          hasActiveTimer: true
        }
      });
    }
    ...
    // CRITICAL: Save to database persistence system
    const saveSuccess = await dbPersistence.saveTimerData(userId, newTimerData);
    if (!saveSuccess) {
      console.error(`❌ Failed to save timer data for user ${userId}`);
      return res.status(500).json({
        success: false,
        message: 'Failed to start timer due to persistence error'
      });
    }

    // Also maintain old system for compatibility
    global.activeTimers[timerId] = newTimerData;

    // VERIFICATION: Check if timer was saved properly
    const verificationData = await dbPersistence.loadTimerData(userId);
    const hasActiveAfterSave = await dbPersistence.hasActiveTimer(userId);

    console.log(`✅ TIMER START SUCCESS - User: ${userId}`);
    console.log(`📊 Verification - HasActive: ${hasActiveAfterSave}, TimerExists: ${verificationData ? 'YES' : 'NO'}`);
    ...
    res.json({
      ...
      data: {
        ...
        enhanced: true,
        verification: {
          saved: !!verificationData,
          hasActiveTimer: hasActiveAfterSave
        }
      }
    });
  } catch (error) {
...
app.post('/api/me/timer/stop', async (req, res) => {
  const userId = 1; // Simplified for this version
  const today = new Date().toISOString().split('T')[0];

  try {
    // Load timer data from database persistence
    const timerData = await dbPersistence.loadTimerData(userId);
    if (!timerData || !timerData.isActive) {
      return res.status(400).json({
        success: false,
    ...
    // Clear timer data
    await dbPersistence.clearTimerData(userId);
    delete global.activeTimers[timerData.timerId];
...
app.get('/api/me/timer/current', async (req, res) => {
  const userId = 1; // Simplified for this version

  try {
    console.log(`🔍 ENHANCED TIMER CHECK for user ${userId}`);

    // Use database persistence to load timer data
    const timerData = await dbPersistence.loadTimerData(userId);
    const hasActiveTimer = await dbPersistence.hasActiveTimer(userId);

    console.log(`📊 Database Check - HasActive: ${hasActiveTimer}, TimerExists: ${timerData ? 'YES' : 'NO'}`);
    ...
    console.log(`✅ ACTIVE TIMER FOUND for user ${userId} via enhanced persistence`);
...
// Get user profile information with dynamic data
app.get('/api/me/profile', authenticateToken, (req, res) => {
  const userId = req.user?.userId || 1;
  const user = currentUserData[userId] || currentUserData[1];
  
  console.log(`🔍 Profile API called for user ${userId}: ${user.full_name}`);
  ...
  res.json({
    ...
    data: {
      user: {
        ...
        email: user.email,
        phone: user.phone,
        profile_photo: `https://api-layer.vercel.app/api/me/profile/photo?user=${user.id}`,
        role: user.role,
        company: "ACME Inc.",
        joined_date: "August 17, 2025",
        employee_id: `EMP00${user.id}`,
        department: user.role === "Developer" ? "Engineering" : user.role === "Designer" ? "Design" : "Engineering",
        status: "Active",
        timezone: "UTC-5",
...
// Update profile information with persistence
app.put('/api/me/profile', authenticateToken, (req, res) => {
  const userId = req.user?.userId || 1;
  const { first_name, last_name, email, phone, timezone } = req.body;
  
  console.log(`📝 Profile update request for user ${userId}:`, { first_name, last_name, email });
  
  // Get current user data
  const currentUser = currentUserData[userId] || currentUserData[1];
  ...
  const updatedUser = {
    ...
    full_name: `${first_name || currentUser.first_name} ${last_name || currentUser.last_name}`,
  ...
  currentUserData[userId] = updatedUser;
  saveUserData();
  
  console.log(`✅ Profile updated for user ${userId}: ${updatedUser.full_name}`);

  res.json({
    success: true,
    message: "Profile updated successfully and persisted",
    data: {
      user: updatedUser,
      persistent: true,
      updated_fields: { first_name, last_name, email, phone, timezone }
    }
  });
});
...
// Test API for user switching and debugging
app.get('/api/test-users', (req, res) => {
  const users = Object.values(currentUserData).map(user => ({
    id: user.id,
    name: user.full_name,
    email: user.email,
    role: user.role,
  ...
  res.json({
    success: true,
    message: "Test users with tokens for debugging",
    data: {
      users,
      currentUserCount: Object.keys(currentUserData).length,
      persistent: true
    }
  });
});

// User switching API for testing
app.post('/api/switch-user', (req, res) => {
  const { userId } = req.body;
  const user = currentUserData[userId];
  ...
  console.log(`🔄 User switched to: ${user.full_name} (ID: ${userId})`);
